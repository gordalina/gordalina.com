...
Today we are happy to announce [Hush](https://github.com/gordalina/hush)!
...
Fast-forward to 2020, I was working on my previous startup and wanted to deploy an API - an Elixir app in a containerized environment and realized that, by default, Elixir apps include a `prod.secrets.exs` which is used to configure an application in a production environment.

This approach would require us to build docker images on a per-environment basis, which would have to include said secrets within them — this was not an ideal scenario.
...
Finally, I'd like want to invite the community to ask any questions, contribute, or submit ideas on [Hush's Issue Tracker](https://github.com/gordalina/hush/issues).
